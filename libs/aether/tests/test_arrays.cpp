...
TEST(ArrayTests, ArrayOfI64) {
  ...
  arr[0] = truk::aether::i64_c(10000000000ULL);
  arr[1] = truk::aether::i64_c(20000000000ULL);
  arr[2] = truk::aether::i64_c(30000000000ULL);

  CHECK_EQUAL(10000000000ULL, arr[0].value());
  CHECK_EQUAL(20000000000ULL, arr[1].value());
  CHECK_EQUAL(30000000000ULL, arr[2].value());
}
...
TEST(ArrayTests, ArrayArithmetic) {
  ...
  CHECK_EQUAL(30, sum.value());

  auto diff = arr[2] - arr[1];
  CHECK_EQUAL(10, diff.value());
}

TEST(ArrayTests, LargeArray) {
  truk::aether::array_c<truk::aether::i8_c> arr(1000);
  CHECK_EQUAL(1000, arr.length());

  arr[0] = truk::aether::i8_c(1);
  arr[999] = truk::aether::i8_c(255);
