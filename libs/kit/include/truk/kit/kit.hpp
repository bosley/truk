...
if no exception, then good. response will be everything caller needs to construct the project

*/

enum class exception_e {
  PARSE_ERROR, // TODO: add errors as needed
  INCLUDE_ERROR, // note: kit is-a lexer/parser for .kit files so we will want to pass source info for markup
  UNKNOWN_ERROR
};
...
public:
  ...
  kit_exception_c(exception_e type, std::size_t at_index, const std::string& message)
  ...
  exception_e type() const noexcept {
    return _type;
  }
...
public:
  ...
  target_application_c(
      ...
      const std::optional<std::vector<std::string>>& library_paths = std::nullopt,
...
public:
  ...
  target_library_c(const std::string& source_entry_file_path,
                   const std::string& output_file_path,
...
std::optional<std::filesystem::path> find_kit_file(const std::filesystem::path& start_dir);

std::filesystem::path resolve_path(const std::filesystem::path& base, const std::string& relative);
