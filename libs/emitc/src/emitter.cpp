...
std::string emitter_c::emit_type(const type_c *type) {
  ...
  if (auto prim = dynamic_cast<const primitive_type_c*>(type)) {
    switch (prim->keyword()) {
      case keywords_e::I8: return "i8";
      case keywords_e::I16: return "i16";
      case keywords_e::I32: return "i32";
      case keywords_e::I64: return "i64";
      case keywords_e::U8: return "u8";
      case keywords_e::U16: return "u16";
      case keywords_e::U32: return "u32";
      case keywords_e::U64: return "u64";
      case keywords_e::F32: return "f32";
      case keywords_e::F64: return "f64";
      case keywords_e::BOOL: return "bool";
      case keywords_e::VOID: return "void";
      default: return "void";
    }
  }
...
void emitter_c::register_variable_type(const std::string& name, const type_c *type) {
...
void emitter_c::visit(const fn_c &node) {
  ...
  for (size_t i = 0; i < node.params().size(); ++i) {
    ...
    if (auto arr = dynamic_cast<const array_type_c*>(param.type.get())) {
      if (arr->size().has_value()) {
        _functions << param_type << " " << param.name.name << "[" << arr->size().value() << "]";
        continue;
      } else {
...
void emitter_c::visit(const var_c &node) {
  ...
  if (_indent_level == 0) {
    ...
    _functions << cdef::indent(_indent_level) << type_str << " " << node.name().name;
  }
...
void emitter_c::visit(const const_c &node) {
  ...
  if (_indent_level == 0) {
    ...
    _functions << cdef::indent(_indent_level) << "const " << type_str << " " << node.name().name;
  }
...
void emitter_c::visit(const binary_op_c &node) {
  ...
  switch (node.op()) {
    case binary_op_e::ADD: _current_expr << " + "; break;
    case binary_op_e::SUB: _current_expr << " - "; break;
    case binary_op_e::MUL: _current_expr << " * "; break;
    case binary_op_e::DIV: _current_expr << " / "; break;
    case binary_op_e::MOD: _current_expr << " % "; break;
    case binary_op_e::EQ: _current_expr << " == "; break;
    case binary_op_e::NE: _current_expr << " != "; break;
    case binary_op_e::LT: _current_expr << " < "; break;
    case binary_op_e::LE: _current_expr << " <= "; break;
    case binary_op_e::GT: _current_expr << " > "; break;
    case binary_op_e::GE: _current_expr << " >= "; break;
    case binary_op_e::AND: _current_expr << " && "; break;
    case binary_op_e::OR: _current_expr << " || "; break;
    case binary_op_e::BITWISE_AND: _current_expr << " & "; break;
    case binary_op_e::BITWISE_OR: _current_expr << " | "; break;
    case binary_op_e::BITWISE_XOR: _current_expr << " ^ "; break;
    case binary_op_e::LEFT_SHIFT: _current_expr << " << "; break;
    case binary_op_e::RIGHT_SHIFT: _current_expr << " >> "; break;
  }
...
void emitter_c::visit(const unary_op_c &node) {
  switch (node.op()) {
    case unary_op_e::NEG: _current_expr << "(-"; break;
    case unary_op_e::NOT: _current_expr << "(!"; break;
    case unary_op_e::BITWISE_NOT: _current_expr << "(~"; break;
    case unary_op_e::ADDRESS_OF: _current_expr << "(&"; break;
    case unary_op_e::DEREF: _current_expr << "(*"; break;
  }
      ...
      switch (builtin->kind) {
        case builtins::builtin_kind_e::ALLOC: {
          if (!node.arguments().empty()) {
            if (auto type_param = dynamic_cast<const type_param_c*>(node.arguments()[0].get())) {
        ...
        case builtins::builtin_kind_e::PANIC: {
          if (!node.arguments().empty()) {
            ...
            _current_expr.str("");
            _current_expr.clear();
            _current_expr << "TRUK_PANIC((" << arg << ").data, (" << arg << ").len)";
            return;
          }
          break;
        }
